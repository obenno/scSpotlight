...
- [ ] **ANAL-01**: Analysis Mode user can load supported Seurat `.Rds`, `.h5ad`, BPCells bundle, and compressed 10x-style inputs and have assay layers converted or preserved as BPCells-backed storage when possible.
- [ ] **ANAL-02**: Analysis Mode user can derive missing normalized/HVG/PCA/neighbors/clusters/UMAP state for processable inputs through memory-conserving processing paths.
- [ ] **ANAL-03**: Analysis Mode user can filter cells, update clustering, and add cell-cycle metadata without preserving dense `scale.data` or triggering unnecessary full-dataset transfers.
