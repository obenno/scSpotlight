...
if (
  ...
  stop(
    "Run benchmarks/run_real_explore_processing_benchmark.R before browser measurement.",
    call. = FALSE
  )
}

port <- suppressWarnings(as.integer(Sys.getenv("SCSPOTLIGHT_BENCHMARK_PORT", "8902")))
...
run_app(
  runningMode = "explore",
  dataDir = dirname(paths$archive_file),
  nCores = 1L,
  options = list(
    port = port,
    host = "127.0.0.1",
