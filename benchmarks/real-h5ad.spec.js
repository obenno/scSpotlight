...
const scratchRoot = process.env.SCSPOTLIGHT_REAL_H5AD_ROOT ||
  path.join(process.env.SCSPOTLIGHT_TEMP_ROOT || "benchmarks/.tmp", "cellxgene-h5ad");
const runId = process.env.SCSPOTLIGHT_REAL_H5AD_RUN_ID || "latest";
...
test("real CellxGene h5ad records Analysis load, first View, selected gene, and RSS gates", async ({ page, browser }) => {
  ...
  try {
    ...
    await dataSelect.fill(descriptor.source_filename);
    const inputOption = page
      .locator(".selectize-dropdown:visible .option")
      .filter({ hasText: descriptor.source_filename });
    await expect(inputOption).toBeVisible();
    ...
    result.browser = {
      method: {
        first_view: "Elapsed wall time from selecting the real h5ad input to a rendered real cell count.",
        selected_gene: "Elapsed wall time from selecting the recorded real-artifact gene to the received client sparkline.",
        server_rss: "Linux /proc sampled every 100 ms for the single-threaded Analysis Mode R Shiny process; VmHWM includes h5ad import and rendering.",
    ...
    result.acceptance = {
      ...
      issue_27_evidence_status: result.acceptance.processing_rss_under_limit &&
