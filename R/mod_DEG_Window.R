...
#' @noRd
mod_DEG_Window_server <- function(id,
                                  seuratObj,
                                  group.by){
    moduleServer(id, function(input, output, session){
        ...
        plot_width <- reactive({
            ...
            if(length(width) != 1 || is.null(width) || !is.finite(width) || width <= 0){
                return(NULL)
            }
            width
        })
        ...
        plot_height <- reactive({
            height <- session$clientData[[paste0("output_", ns("heatmap"), "_height")]]
            if(length(height) != 1 || is.null(height) || !is.finite(height) || height <= 0){
                return(NULL)
            }
            height
        })
        ...
        output$heatmap <- renderPlot(
            ...
            height <- plot_height()
                ...
                validate(
                    need(isTruthy(obj), "Please input single cell data before generating DEG heatmap"),
                    need(isTruthy(group_by_value) && group_by_value != "None", "Choose a grouping metadata column before running DEG analysis"),
                    need(isTruthy(markers) && nrow(markers) > 0, 'Please use "Find Markers" to generate DEG marker list'),
            ...
            validate(
                need(!is.null(fc_col), "DEG results do not include a supported log fold-change column"),
                need(!is.null(adj_p_col), "DEG results do not include an adjusted p-value column")
            )

            markers_filtered <- markers[markers[[adj_p_col]] < p_adj_cutoff, , drop = FALSE]

            validate(
                need(nrow(markers_filtered) > 0, paste0("No DEG features pass adjusted p-value < ", p_adj_cutoff))
            )
