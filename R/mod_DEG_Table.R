...
#'
#' @noRd
mod_DEG_Table_server <- function(id, DEG_markers, pAdjCutoff) {
    ...
    output$DEG_list <- renderDT({
      ...
      validate(
        need(
          ...
          paste0("No DEG rows pass adjusted p-value < ", cutoff)
        )
      )
