...
#'
#' @noRd
mod_Download_server <- function(id, seuratObj) {
  moduleServer(id, function(input, output, session) {
    ...
    should_warn_rds_export <- function(object) {
      ...
      is_large_object <- tryCatch(ncol(object) >= 1e+05, error = function(...) {
        FALSE
      })
    ...
    observe({
      ...
      if (runningMode == "analysis" && isTruthy(seuratObj())) {
        downloadFormat <- c(
          "Explore Parquet",
          "BPCells",
          "h5ad",
      ...
      updateSelectizeInput(
        ...
        label = "Result Format",
        selected = if (length(downloadFormat)) {
          downloadFormat[[1]]
        } else {
          character(0)
        },
    ...
    observeEvent(
      input$downloadTrigger,
      {
        ...
        if (
          !identical(input$downloadFormat, "Rds") ||
            !should_warn_rds_export(obj)
        ) {
          trigger_download()
          return()
        }
    ...
    output$downloadData <- downloadHandler(
      filename = function() {
        ...
        prefix <- "scSpotlight."
        outFile <- case_when(
          input$downloadFormat == "metaData" ~ paste0(
            prefix,
            "metaData.",
            Sys.Date(),
            ".tsv.gz"
          ),
          input$downloadFormat == "Explore Parquet" ~ paste0(
            prefix,
            Sys.Date(),
            ".explore-parquet.zip"
          ),
          input$downloadFormat == "BPCells" ~ paste0(
            prefix,
            Sys.Date(),
            ".bpcells.zip"
          ),
      ...
      content = function(file) {
        ...
        if (str_detect(file, "\\.Rds$")) {
          ...
          progressr::withProgressShiny(
            {
              bundle_progress <- progressr::progressor(steps = 3)
              bundle_progress(message = "Writing Explore Parquet bundle")
            ...
            message = "Saving Explore Parquet bundle...",
...
#'
#' @noRd
showSpinnerNotification <- function(
  message,
  action = NULL,
  duration = NULL,
  closeButton = FALSE,
