...
pixi_available <- names(pixi_candidates)[vapply(
  seq_along(pixi_candidates),
  function(i) {
    pixi_has_exact_package(
      pixi_candidates[[i]],
      pixi_channels[[i]],
      platform = platform
    )
...
if (length(pixi_available)) {
  ...
  if (pixi_success) {
    ...
    message(
      "Pixi-native add was unavailable or failed; falling back to pak for all optional packages."
    )
  }
} else {
  message(
    "No exact conda-channel packages found for the optional package set on this platform; using pak fallback."
  )
}
