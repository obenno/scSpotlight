...
- Run `pixi run install-optional-packages` when you want optional Seurat performance helpers such as `BPCells` and `presto`. It prefers Pixi-native conda-forge installs and then uses `pak` as a fallback.
...
- `r-duckdb` is available from conda-forge for Linux and macOS Intel in the current R 4.5 stack. macOS Apple Silicon and Windows still fall back to `pak` for `duckdb` in this repo's setup flow.
...
- `pixi run install-optional-packages` - install optional performance packages such as `BPCells` and `presto`
